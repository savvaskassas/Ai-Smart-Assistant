...
from app.productivity.analyzer import analyze_productivity
...
def get_detailed_productivity_analysis():
    """
    Get comprehensive productivity analysis using the analyzer module.
    """
    creds = None
    if os.path.exists(TOKEN_FILE):
        creds = Credentials.from_authorized_user_file(TOKEN_FILE, SCOPES)
    ...
    # Get events from the past week for better analysis
    now = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
    ...
    events_result = service.events().list(
        calendarId='primary', timeMin=week_ago_iso, timeMax=now_iso, singleEvents=True,
        orderBy='startTime', maxResults=200).execute()
    events = events_result.get('items', [])
    
    # Analyze productivity
    analysis = analyze_productivity(events)
